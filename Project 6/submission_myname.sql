...
RANK() OVER(PARTITION BY o.state ORDER BY vehicle_maker) Vehicle_rank
FROM ord_cust_v o 
...
SELECT COUNT(order_id) total_orders, SUM(calc_revenue_f(vehicle_price, quantity,discount)) 
AS revenue, quarter_number 
FROM ord_cust_v 
GROUP BY quarter_number 
ORDER BY quarter_number;
